...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      // In demo mode, we'll use the credentials provider directly
      // In production, you would create a user in the database first
      ...
      <Card className="w-full max-w-md bg-zinc-900 border-zinc-800">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl font-black">
            {step === 1 ? "Создать аккаунт" : "Настройка профиля"}
          </CardTitle>
          <CardDescription className="text-zinc-500">
            {step === 1
              ? "Зарегистрируйтесь, чтобы найти партнёров"
              : "Укажите свой уровень игры"}
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          {step === 1 ? (
            ...
            <form onSubmit={handleRegister} className="space-y-6">
              ...
              {/* Skill Level */}
              <div className="space-y-4">
                <Label className="text-zinc-400">Ваш уровень игры (NTRP)</Label>
